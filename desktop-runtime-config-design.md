...
当前桌面端在主进程启动时读取 `~/.multica/desktop.json`：文件不存在时必须先配置私有化后端，文件存在但无效时阻塞启动并要求重新配置。桌面端仅面向私有化部署，不应在未配置时连接公网服务。

目标是让首次打开桌面端的用户在登录前完成私有化后端地址配置，同时保持现有 API/WS 初始化顺序、守护进程 profile 隔离和已配置用户的无感启动。
...
3. **输入私有化地址**：页面直接显示必填“后端地址”输入框，支持粘贴内网域名/IP 和端口，不提供 Cloud 选项。
...
5. **保存并继续**：保存按钮在格式无误后可用。主进程规范化 URL，补齐 `wsUrl`/`appUrl`，原子写入 `~/.multica/desktop.json`（先写临时文件再 rename），成功后返回新配置并重载窗口。重载后正常进入现有登录页。
...
- `apps/desktop/src/main/runtime-config-loader.ts` 返回 `source`/`needsSetup`，缺失配置必须阻塞 renderer，不能回退到 Cloud 默认。
...
- 文案进入 `packages/views/locales/{en,zh-Hans,ko,ja}`，至少覆盖标题、地址输入、校验、连接状态、写入失败和重启提示。
- 采用键盘可达顺序（地址 → 测试 → 保存），错误通过 `aria-describedby` 关联；不把 URL 放到 telemetry 或错误上报中。
...
- 桌面端构建默认启用私有化配置门禁：缺失配置只能进入私有地址表单，避免客户端把数据发往公网。
...
- 新增页面测试：首次运行阻塞 CoreProvider、仅显示私有地址表单、字段校验、测试连接 loading/成功/失败、保存后触发重载；invalid 状态的“重新配置”路径。
- 手工验收矩阵：全新用户、已有内网配置、损坏 JSON、无写权限、内网离线、自签名证书、Windows/macOS/Linux 路径和中文/英文 locale。

## 分阶段发布

1. 先落地主进程配置状态和 IPC，默认不改变已有 `configured` 用户。
2. 发布首次配置页，所有桌面构建均要求填写私有化后端地址。
