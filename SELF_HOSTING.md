...
- **External messaging integrations.** Slack, Lark, DingTalk, WeCom, and Telegram need outbound access to their providers; their current socket/long-polling implementations do not need public inbound webhooks. For an isolated network, set `MULTICA_MESSAGING_INTEGRATIONS_ENABLED=false` in the API environment. This stops provider connections and bindings and hides their Settings and agent-detail entries, even if encryption keys remain configured. Saved installations and credentials are retained. The switch defaults to `true` when unset; after changing it, recreate the API container with `docker compose -f docker-compose.selfhost.yml up -d backend` and refresh clients. In Helm, set `backend.config.messagingIntegrationsEnabled: false`.
- **Self-hosted Git remains available.** Keep `MULTICA_VCS_INTEGRATION_ENABLED=true` and provide `MULTICA_VCS_SECRET_KEY` to connect an intranet GitLab, Gitea, or Forgejo instance. Multica and the Git server must be able to reach each other's intranet API/webhook addresses. GitHub.com integration and skill imports from GitHub still require internet access.
