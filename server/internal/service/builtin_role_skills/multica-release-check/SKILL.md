...
7. **供应链** — 固定依赖锁文件和来源，核对构建产物 artifact digest、签名/校验和以及可生成的 SBOM；无法验证来源时视为未通过。
8. **恢复** — 对数据或状态变更记录备份点、RPO/RTO、恢复演练结果、降级和重试限制；“回退提交”不能代替数据恢复证据。
...
如果发布包含数据变更却没有备份点、恢复演练或 RPO/RTO 证据，发布门禁必须保持 hold；“回退提交”不是恢复证明。
