...
| Validates and normalizes `config.presentation` (`category` / `icon` only); seeds it from frontmatter `metadata.category` / `metadata.icon` on import | `skill.ValidatePresentation` / `NormalizePresentation` / `PresentationFromFrontmatter` in `server/internal/skill/presentation.go`; called from `CreateSkill`, `UpdateSkill`, `finishSkillImport` and the overwrite branch of `resolveImportSkillConflict` in `server/internal/handler/skill.go` |
| Skill labels are workspace labels: `label_ids` on create are checked (`GetLabel`, `resource_type == "skill"`) then attached in the create tx; the list embeds `labels` per skill | `CreateSkill` / `labelsBySkill` in `server/internal/handler/skill.go`; `createSkillWithFilesInTx` in `server/internal/handler/skill_create.go`; `ListLabelsForSkills` / `AttachLabelToSkill` in `server/pkg/db/queries/issue_label.sql` |
