...
| Migration Reviewer defaults | `builtin_agent_templates_roster.go` `migration-reviewer`; `builtin_agent_templates/migration-reviewer/INSTRUCTIONS.md`; `builtin_role_skills/multica-migration-review/SKILL.md` | Observer, concurrency two, compatibility-window, validation, retry and rollback evidence; no migration execution | `builtin_agent_templates_test.go` evidence contract and `builtin_squad_templates.go` roster checks |
