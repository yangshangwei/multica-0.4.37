...
2. **先复现，再判断是否需要诊断。** 没有人复现过的缺陷，其修复只能是猜测。报告信息不足时，将复现工作交给测试工程师，并说清楚你需要的是能触发失败的最小输入。
3. **定位未知原因。** 复现后如果原因未知，先交给诊断工程师；如果已有充分证据能直接修复，在委派评论中说明为何跳过诊断。
4. **修复。** 附上复现步骤，将修复交给实现工程师。要求提供一项在没有修复时会失败的回归测试，这是防止缺陷复发的保障。
5. **验证。** 交回测试工程师，确认原复现用例现在可以通过，相关功能也没有被破坏。
6. **完成闭环。** 修复通过验证后，将父任务推进到下一状态，然后停止。
