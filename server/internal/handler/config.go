...
type AppConfig struct {
	...
	// DeviceAuthAvailable tells a client this deployment will mint a session
	// from a device id alone (POST /auth/device), so a desktop app with no
	// stored token can boot straight into the workspace instead of rendering
	// the login page. Omitted when false so the managed cloud and every
	// deployment with the switch off keep their previous response shape, and
	// absent MUST read as false on the client: guessing "available" would
	// replace a working login page with a request that can only 403.
