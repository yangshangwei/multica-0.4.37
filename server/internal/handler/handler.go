...
type Config struct {
	...
	// DeviceAuthEnabled turns on the intranet device-login path
	// (POST /auth/device): a client presenting a stable device id gets a JWT
	// and, the first time that id is seen, a user plus membership in the
	// shared workspace named below. It exists for intranet deployments with
	// no mail relay to deliver a login code and no reachable Google OAuth,
	// where the alternative to this is not "log in properly" but "cannot use
	// the app at all".
	//
