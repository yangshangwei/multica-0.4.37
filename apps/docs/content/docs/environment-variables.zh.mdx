...
还需要配置 SMTP 邮件服务，否则验证码和邀请只会写入服务端日志。
...
桌面端和 Web 的“帮助 → 变更说明”从当前部署读取发布记录，无需访问公网。默认使用 API 构建中包含的记录；如需在不重启 API 的情况下发布新记录，配置以下变量。

| 变量 | 默认值 | 说明 |
|---|---|---|
| `CHANGELOG_FILE` | 空 | API 可读取的累计日志 JSON 文件；为空时使用内置记录 |
| `CHANGELOG_DIRECTORY` | `./changelog` | Docker Compose 挂载的宿主机目录；容器内对应 `/app/data/changelog` |

使用 Docker Compose 时，在 `.env` 中设置 `CHANGELOG_FILE=/app/data/changelog/changelog.json`。先将有效的发版日志放入宿主机目录，再通过 `docker compose up -d backend` 使路径配置生效。之后每次发版只需原子替换文件，已打开的页面会在 60 秒内发起下一次检查，也可以手动刷新。

日志由发版工具从提交记录生成，并保留以前的正式版、预发布版和官方来源说明。不要直接编辑正在使用的文件；有 Node 的发布机可以运行 `node scripts/publish-changelog.mjs --input <发版产物>/changelog.json --destination ./changelog/changelog.json`。离线安装和升级包会通过已导入的前端镜像运行发布器，目标机无需额外安装 Node。

挂载整个目录，不要把单个文件作为 bind mount 或 Kubernetes `subPath` 挂载，否则原子替换后容器可能仍读取旧文件。Helm 部署可用 `backend.changelog.existingClaim` 挂载已有的日志卷，并将 `backend.config.changelogFile` 设为 `/app/data/changelog/changelog.json`；多副本需要所有 API 实例可读的共享卷。

文件最大为 2 MiB。文件不可读或格式错误时，API 保留当前进程最后读取成功的内容，并返回 `is_stale: true`；还没有成功读取过文件时，则显示内置记录并提示尚未同步。首次启用路径配置需要重建容器，后续更新文件不需要。内网客户端只能看到已经交付到本部署的记录。
...
| `MULTICA_PUBLIC_URL` | 空 | 对外 API 地址；用于 webhook URL 和运行时连接说明 |
| `MULTICA_DAEMON_SERVER_URL` | 回退到 `MULTICA_PUBLIC_URL`，再回退到 `MULTICA_APP_URL` / `FRONTEND_ORIGIN` | API 进程写入 `multica setup self-host` 命令的服务端地址；守护进程访问 API 的地址与对外 webhook 地址不同时设置 |
...
| `SMTP_PASSWORD` | 空 | 密码 |
| `SMTP_FROM_EMAIL` | 空 | Envelope From 与邮件 From，建议显式设置 |
...
| `ATTACHMENT_DOWNLOAD_MODE` | `auto` | `auto`、`cloudfront`（内网部署不可用）、`presign` 或 `proxy` |
...
| `RATE_LIMIT_AUTH` | `5` | 单 IP 每分钟发送验证码的次数 |
...
完全隔离的内网部署，在 API 服务的环境变量中设置：

```dotenv
MULTICA_MESSAGING_INTEGRATIONS_ENABLED=false
MULTICA_VCS_INTEGRATION_ENABLED=true
```

第一项统一关闭飞书、Slack、钉钉、企业微信和 Telegram 的后台连接及绑定接口，设置页和智能体详情也会隐藏对应入口。已保存的安装记录和凭据会保留。开关未设置时默认为 `true`；恢复为 `true` 并重启 API 服务后，仍需各渠道的加密密钥和可用网络才能连接。

第二项独立保留内网 GitLab、Gitea、Forgejo 集成；连接时还需设置 `MULTICA_VCS_SECRET_KEY`，并确保 Multica 与 Git 服务在内网互通。修改 Compose 的 `.env` 后，执行 `docker compose -f docker-compose.selfhost.yml up -d backend` 重新创建 API 容器，并刷新客户端。Helm 部署对应 `backend.config.messagingIntegrationsEnabled: false` 与 `backend.config.vcsIntegrationEnabled: true`。

| 集成 | 变量 | 说明 |
|---|---|---|
| 外部通讯 | `MULTICA_MESSAGING_INTEGRATIONS_ENABLED` | 默认 `true`；设为 `false` 统一关闭五种通讯集成，不影响自托管 Git |
...
自托管 Git 集成（Forgejo、Gitea、GitLab）的配置步骤见[自托管 Git 代码托管](/vcs-integration)。
...
PostHog 的默认地址在内网不可达。内网部署建议直接关闭统计上报：

```dotenv
ANALYTICS_DISABLED=true
```

如果内网自建了 PostHog，也可以接入：

| 变量 | 默认值 | 说明 |
|---|---|---|
| `ANALYTICS_DISABLED` | `false` | 设为 `true` 关闭 PostHog 上报；内网部署建议开启 |
| `POSTHOG_API_KEY` | 空 | 不设置时统计上报关闭；接入自己的 PostHog 项目时填写 |
| `POSTHOG_HOST` | `https://us.i.posthog.com` | PostHog 地址；内网自建时填写内网地址 |
