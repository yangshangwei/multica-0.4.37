...
Desktop checks the release feed it was built with, which is fine for the public release but wrong for a self-hosted deployment that ships its own Desktop builds. Add `updateUrl` to point the updater at a static directory you control:
...
`updateUrl` must use `http` or `https` and must not contain credentials. The directory serves the files `pnpm package` already produces, unchanged: `latest.yml` (`latest-arm64.yml` on arm64) and the `.exe` plus `.exe.blockmap` for Windows, `latest-mac.yml` (`latest-x64-mac.yml` on Intel) with the `.zip` and `.blockmap` for macOS, and `latest-linux.yml` with the AppImage for Linux. Any static file server works; keep the previous version's `.blockmap` next to the new one so clients download only the changed blocks. Leaving `updateUrl` out restores the built-in feed. Saving a server address from the login page keeps this field as it was.
