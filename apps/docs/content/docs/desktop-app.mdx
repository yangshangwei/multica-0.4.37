...
Auto-update is on by default. An intranet deployment can use an [administrator-provided update directory](#taking-updates-from-your-own-server). The app checks for and downloads new versions in the background; once a download finishes you can restart to install immediately or let it install on the next quit. You can also turn off automatic checks or check for new versions manually under **Settings → Updates**. Turning off automatic checks does not cancel an update that has already downloaded.

Updates are distributed per OS and CPU architecture:

- Windows arm64, Windows ia32 (32-bit), and macOS x64 (Intel) each use their own update feed;
...
- if the update source is unavailable or auto-update fails, quit Desktop and install over the top with the matching installer supplied by your administrator or the downloads page.

## Connecting to a self-hosted instance

Desktop requires a server address on first launch. Enter the address supplied by your administrator in the connection settings, or create `desktop.json` inside the `.multica` directory of your home directory:
...
### Taking updates from your own server

An administrator can provide a static HTTP/HTTPS update directory, including on an intranet. Quit Desktop completely and back up the existing `desktop.json`. Preserve the business server addresses and other settings, and add or change only `updateUrl`. Replace the placeholders below with the addresses provided by your administrator:

```json
{
  "schemaVersion": 1,
  "apiUrl": "https://<intranet-api-host>",
  "updateUrl": "https://<intranet-download-host>/desktop"
}
```

`updateUrl` points to the directory containing update metadata, not an installer file or a download page. It must use `http` or `https` and must not contain credentials; query parameters and fragments are removed. Clients must trust the certificate when using HTTPS. The service must return files directly, without redirecting to a browser login page.

Publish the files referenced by the generated metadata and their generated blockmaps, preserving the original names:

| Platform and architecture | Update metadata | Auto-update artifacts |
| --- | --- | --- |
| Windows x64 / ARM64 / ia32 | `latest.yml` / `latest-arm64.yml` / `latest-ia32.yml` | `.exe`, `.exe.blockmap` |
| macOS Apple Silicon / Intel | `latest-mac.yml` / `latest-x64-mac.yml` | `.zip` and generated blockmaps; `.dmg` is for manual installation |
| Linux x64 / ARM64 | `latest-linux.yml` / `latest-linux-arm64.yml` | The matching `.AppImage` |

Keep older blockmaps to support differential downloads; clients may still fall back to a full download. Validate and publish installers before switching metadata so clients never discover a version whose files are unavailable.

Restart Desktop completely, then check under **Settings → Updates**. `apiUrl` and `updateUrl` are independent: omitting `updateUrl` uses the built-in release feed, not the business server. Saving a business server address from the login page preserves this field. This file configuration applies to installed clients; development mode uses environment variables and cannot substitute for an installed-client upgrade test.

Manual file changes are read at startup. Configuration failures fall into two cases:

- **File not found** (wrong directory, or a filename that isn't exactly `desktop.json`) — Desktop requires server setup and does not automatically connect to Cloud.
- **File found but invalid** JSON, version, or URLs — Desktop shows a configuration error; correct the file to continue.

Deleting the file and restarting requires server setup again. To restore only the built-in update source, keep the file and remove `updateUrl`.
