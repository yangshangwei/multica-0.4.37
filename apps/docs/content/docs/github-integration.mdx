...
**Settings → Integrations → GitHub** has four switches:
...
Clicking **Disconnect** in **Settings → Integrations → GitHub** only removes the relationship between the current Multica workspace and the installation; it does not uninstall the App from GitHub for you. Existing PR records are kept, and new events stop flowing into that workspace.
...
Restart the API server, then complete the connection in **Settings → Integrations → GitHub**.
