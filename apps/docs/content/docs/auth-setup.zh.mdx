---
title: 登录与注册
description: 配置邮箱验证码和新用户注册范围。
---

import { Callout } from "fumadocs-ui/components/callout";

Multica 默认使用邮箱验证码登录。已经存在的用户始终可以再次登录；注册限制只决定是否允许创建新账号。
...
如果没有设置 `SMTP_FROM_EMAIL`，建议显式设置，避免发件地址不符合内网邮件网关的要求。私有 CA 或自签证书需要把 CA 加入容器信任库；`SMTP_TLS_INSECURE=true` 会跳过证书校验，只应在可信内网中临时使用。
...
启动日志会说明当前使用的是 `SMTP relay` 还是 `DEV mode`。
...
不要在多人可访问的实例中启用固定验证码。生产环境的组合是 `APP_ENV=production` 加空的 `MULTICA_DEV_VERIFICATION_CODE`。
</Callout>
