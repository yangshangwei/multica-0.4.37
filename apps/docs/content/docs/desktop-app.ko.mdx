...
Desktop은 빌드할 때 포함된 릴리스 피드를 확인합니다. 공개 릴리스에는 문제가 없지만, 자체 호스팅 환경에서 직접 빌드한 Desktop을 배포한다면 업데이트 경로를 직접 관리하는 정적 디렉터리로 돌려야 합니다. `updateUrl`을 추가하세요:
...
`updateUrl`은 `http` 또는 `https`를 사용해야 하며 자격 증명을 포함할 수 없습니다. 디렉터리에는 `pnpm package`가 만드는 파일을 그대로 두면 됩니다. Windows는 `latest.yml`(arm64는 `latest-arm64.yml`)과 `.exe`, `.exe.blockmap`, macOS는 `latest-mac.yml`(Intel은 `latest-x64-mac.yml`)과 `.zip`, `.blockmap`, Linux는 `latest-linux.yml`과 AppImage입니다. 어떤 정적 파일 서버든 사용할 수 있습니다. 이전 버전의 `.blockmap`을 새 버전 옆에 두면 클라이언트는 바뀐 블록만 내려받습니다. `updateUrl`을 생략하면 기본 피드로 돌아갑니다. 로그인 페이지에서 서버 주소를 저장해도 이 필드는 그대로 유지됩니다.
