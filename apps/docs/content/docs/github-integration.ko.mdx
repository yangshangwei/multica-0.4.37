...
**설정 → 연동 → GitHub**에는 네 가지 스위치가 있습니다.
...
**설정 → 연동 → GitHub**에서 **연결 해제**를 클릭하면 현재 Multica 워크스페이스와 해당 installation의 관계만 제거됩니다. GitHub에서 App을 대신 제거하지는 않습니다. 기존 PR 기록은 유지되고 새 이벤트만 해당 워크스페이스에 들어오지 않습니다.
...
API 서비스를 다시 시작한 뒤 **설정 → 연동 → GitHub**에서 연결을 완료합니다.
