...
Desktop はビルド時に組み込まれたリリースフィードを確認します。公開リリースではそれで問題ありませんが、セルフホスト環境で独自の Desktop ビルドを配布する場合は、更新先を自分が管理する静的ディレクトリに向ける必要があります。`updateUrl` を追加してください:
...
`updateUrl` は `http` または `https` を使い、認証情報を含めてはいけません。ディレクトリには `pnpm package` が生成するファイルをそのまま置きます: Windows は `latest.yml`（arm64 は `latest-arm64.yml`）と `.exe`、`.exe.blockmap`、macOS は `latest-mac.yml`（Intel は `latest-x64-mac.yml`）と `.zip`、`.blockmap`、Linux は `latest-linux.yml` と AppImage です。静的ファイルサーバーであれば何でも構いません。前バージョンの `.blockmap` を新バージョンと並べて置くと、クライアントは変更されたブロックだけをダウンロードします。`updateUrl` を省略すると組み込みのフィードに戻ります。ログインページでサーバーアドレスを保存してもこのフィールドは変わりません。
