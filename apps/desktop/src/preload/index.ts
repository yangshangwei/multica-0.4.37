...
function fetchDeviceIdentity(): DeviceIdentity | null {
  try {
    const identity = ipcRenderer.sendSync("device-identity:get") as
      | DeviceIdentity
      | null
      | undefined;
    if (identity && typeof identity.deviceId === "string") return identity;
  } catch {
    // Main could not resolve one. The renderer treats a missing identity as
    // "no device auth", which is the login page — the same place it would end
    // up on a deployment that does not offer it.
  }
  return null;
}
...
const deviceIdentity = fetchDeviceIdentity();
