...
// Resolved once at startup and handed to the renderer at preload time, so the
// first render already knows which machine it is. Only used on deployments
// that declare device auth; elsewhere the renderer ignores it.
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    // this during its first render to decide whether it can boot into a
    // session instead of the login page.
    ipcMain.on("device-identity:get", (event) => {
      event.returnValue = deviceIdentity;
    });
