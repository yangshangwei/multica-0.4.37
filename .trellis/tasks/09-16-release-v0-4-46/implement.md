...
- [ ] Commit verified changes using Lore trailers and push the main candidate.
...
- [ ] Build the requested offline intranet upgrade installer and Windows desktop installer; validate contents and native Windows installation.
- [ ] Perform real previous-version to v0.4.46 offline upgrade with data/configuration persistence checks.
- [ ] Verify published release assets against local SHA-256 values, document limitations, and deliver download paths plus test report.
- [ ] Final requirement-by-requirement audit; archive task / persist evidence and mark goal complete only when all requirements are proven.
