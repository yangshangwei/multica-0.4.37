...
- [x] 统计报告说明新增角色的触发阈值、近期开工量和独立交付物；未达阈值时测试确认不出现在默认 roster。
- [x] 体验角色样例包含浏览器路径、可访问性检查、失败截图/日志和跨端差异结论。
- [x] 迁移角色样例包含旧/新契约、兼容期、校验查询、回滚条件和人工审批点。
- [x] 两个角色的模板、skill、本地化、权限和文档在模板测试中保持一致。
