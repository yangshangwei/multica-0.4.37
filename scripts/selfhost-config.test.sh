...
printf 'MULTICA_DEVICE_AUTH_ENABLED=true\nMULTICA_DEVICE_AUTH_WORKSPACE=lab\nMULTICA_DEVICE_AUTH_WORKSPACE_NAME=Lab-Intranet\nMULTICA_DEVICE_AUTH_ROLE=admin\n' >>"$tmp_env"
...
# Same allowlist hazard for intranet device auth, with a worse failure mode: an
# operator who sets MULTICA_DEVICE_AUTH_ENABLED and never sees it reach the
# container gets a server that still demands a login, on a network where no
# login can be completed, and nothing anywhere says why.
require_config "$config" 'MULTICA_DEVICE_AUTH_ENABLED: "true"'
require_config "$config" 'MULTICA_DEVICE_AUTH_WORKSPACE: lab'
require_config "$config" 'MULTICA_DEVICE_AUTH_WORKSPACE_NAME: Lab-Intranet'
require_config "$config" 'MULTICA_DEVICE_AUTH_ROLE: admin'

while IFS= read -r device_var; do
  if ! grep -Eq "^[[:space:]]+${device_var}: \\\$\{${device_var}:-" docker-compose.selfhost.yml; then
    echo "$device_var is documented in .env.example but not mapped into the backend"
    echo "service in docker-compose.selfhost.yml, so self-hosted deployments cannot set it."
    exit 1
  fi
...
# The air-gapped bundle carries the only copies of these images that will ever
# reach the offline network, so it has to save exactly what the compose files
# start. It reads all three names back out of them; a rename or a database bump
# that the script cannot parse must fail here, not after a technician has
# already carried a tarball with a missing image across the air gap.
if [ ! -x scripts/offline-bundle.sh ]; then
  echo "scripts/offline-bundle.sh is missing or not executable, but SELF_HOSTING.md"
  echo "documents 'make offline-bundle' as the air-gapped install path."
  exit 1
fi

if ! grep -Fq 'bash scripts/offline-bundle.sh' Makefile; then
  echo "The offline-bundle make target no longer calls scripts/offline-bundle.sh."
  exit 1
fi
...
for bundled in "$compose_db_image" multica-backend:dev multica-web:dev; do
  if ! grep -Fq "$bundled" <<<"$bundle_plan"; then
    echo "The offline bundle does not include $bundled:"
    echo "$bundle_plan"
    exit 1
  fi
done
