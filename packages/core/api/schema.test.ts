...
describe("ApiClient schema fallback", () => {
  ...
  describe("getConfig", () => {
    ...
    // session from a device id". Guessing the other way swaps a working login
    // page for a request that can only 403.
    it("fails closed on device auth when the server omits or garbles the flag", async () => {
      stubFetchJson({ cdn_domain: "", allow_signup: true });
      const client = new ApiClient("https://api.example.test");
      expect((await client.getConfig()).device_auth_available).not.toBe(true);

      stubFetchJson({ cdn_domain: "", allow_signup: true, device_auth_available: "yes" });
      expect((await client.getConfig()).device_auth_available).not.toBe(true);
    });

    it("reports device auth when the server declares it", async () => {
      stubFetchJson({ cdn_domain: "", allow_signup: true, device_auth_available: true });
      const client = new ApiClient("https://api.example.test");
      expect((await client.getConfig()).device_auth_available).toBe(true);
    });
  });

  describe("deviceLogin", () => {
    it("returns an empty session when the response is malformed", async () => {
      // An unusable body must not arrive as a half-parsed session: the caller
      // would persist an empty token and then 401 on everything.
      stubFetchJson({ token: 42, user: "not-an-object" });
      const client = new ApiClient("https://api.example.test");
      await expect(client.deviceLogin("a".repeat(32), "mac-mini")).resolves.toEqual({
        token: "",
        user: expect.objectContaining({ id: "" }),
      });
    });
    ...
    it("parses a well-formed session", async () => {
      stubFetchJson({
        token: "jwt-token",
        user: { id: "user-1", name: "artisan@mac-mini", email: "device-x@device.multica.local" },
      });
      const client = new ApiClient("https://api.example.test");
      const session = await client.deviceLogin("b".repeat(32));
      expect(session.token).toBe("jwt-token");
      expect(session.user.id).toBe("user-1");
      expect(session.user.name).toBe("artisan@mac-mini");
    });
  });
