...
export interface AppConfigResponse {
   ...
   * (`POST /auth/device`), so an intranet client with no stored token can boot
   * straight into the app. Absent on every server that predates the switch and
   * on every deployment with it off — and the endpoint answers 403 in both
   * cases, so absent must read as false rather than "try it and see". */
  device_auth_available?: boolean;
...
export const AppConfigSchema = z.object({
  ...
  device_auth_available: BooleanWithDefaultSchema(false).optional(),
...
export const EMPTY_APP_CONFIG: AppConfigResponse = {
  ...
  // Fail closed: guessing "available" would swap a working login page for a
  // request that can only 403.
  device_auth_available: false,
  feature_flags: {},
};
...
// Login responses (`/auth/verify-code`, `/auth/google`, `/auth/device` POST).
// The token is what every later request depends on, so an unreadable body must
// come back as an empty one — a caller that stored a partially-parsed session
// would look logged in and 401 on everything.
export const LoginResponseSchema = z.object({
  token: z.string().default(""),
  user: UserSchema,
}).loose();

// Typed structurally rather than as ApiClient's LoginResponse: that interface
// lives in client.ts, which imports this module.
