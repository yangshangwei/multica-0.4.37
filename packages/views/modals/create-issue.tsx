...
export function ManualCreatePanel({
  ...
  // Toolbar fields hidden via Settings → Preferences reuse the overflow reveal
  ...
  // Inline pill reveal per toolbar field: kept by Settings → Preferences, holding a
  ...
  return (
    <>
            ...
            {/* Property toolbar — each field renders per the Settings → Preferences
            ...
            <div className="flex items-center gap-1.5 px-4 py-2 shrink-0 flex-wrap">
              ...
              {/* Start date — collapsed into the ⋯ menu by default since it's
                  a low-frequency field (exposable via Settings → Preferences).
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-auto">
                  {/* Re-entry points for toolbar fields hidden via
                      Settings → Preferences. Listed in toolbar order; each opens
                  ...
                  <DropdownMenuSeparator />
                  {/* Field visibility lives in Settings → Preferences; the
                      modal closes first so the dialog doesn't linger over the
                      settings page. The draft store already holds everything
                      typed, so nothing is lost across the round-trip. */}
                  <DropdownMenuItem
                    render={
                      <AppLink
                        href={`${p.settings()}?tab=preferences`}
                        onClick={(e) => {
