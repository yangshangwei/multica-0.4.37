...
vi.mock("../issues/components", () => ({
  ...
  // Labels can now be hidden via Settings → Preferences and revealed from the
...
describe("CreateIssueModal", () => {
  ...
  it("hides toolbar fields turned off in Settings → Preferences and re-reveals them from the overflow", async () => {
    const user = userEvent.setup();
  ...
  it("renders due date inline when enabled in Settings → Preferences", () => {
  ...
  it("routes Customize fields to Settings → Preferences and closes the dialog", async () => {
    const user = userEvent.setup();
    const onClose = vi.fn();
    ...
    expect(onClose).toHaveBeenCalled();
    expect(mockPush).toHaveBeenCalledWith("/ws-test/settings?tab=preferences");
  });
