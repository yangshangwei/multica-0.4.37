...
offline-upgrade-bundle: ## Build a versioned tarball for upgrading an offline server
	$(REQUIRE_COMPOSE)
	@bash scripts/build-offline-upgrade.sh $(if $(OUT),--output $(OUT),) $(if $(PLATFORM),--platform $(PLATFORM),)
...
check: ## Run static checks, bounded TS/Go tests, and isolated production Web E2E
	$(REQUIRE_ENV)
